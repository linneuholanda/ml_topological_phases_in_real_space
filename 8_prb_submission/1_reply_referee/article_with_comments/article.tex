...
\section{Topological feature engineering}
\label{topological_feature_engineering}

In this last section, we show how the theoretical formalism sketched in section \ref{emergent_information_entropy_wave_functions} can be used to instigate yet another round of machine learning. As we have argued at the end of section \ref{information_entropy_signatures}, the information entropy signatures can be used as a powerful dimensionality reduction tool: by keeping only the most relevant lattice sites in an information entropy signature, the amount of data needed to characterize the topological phase of an SSH system can be largely decreased with little loss of information. Perhaps more interesting, nonetheless, are the possibilities that the information entropy signatures in real and wavevector space offer when used complementarily. Here we explore the engineering of new topological features in real space. 
...
Contrary to their real space counterparts, the wavevector space information entropy signatures are highly localized, which by itself does not come as a surprise. This gives us an opportunity to engineer new features in real space that carry more relevant topological information than the simpler strategy of selecting the most important lattice sites in a real space information entropy signature. We may write equation \ref{discrete_fourier_transform} in matrix form as 
\begin{equation}
\label{dft}
...
When trying to engineer informative global topological features in real space, the information entropy signatures in wavevector space provide good hints of where to start.  Figures \ref{fourier_feature_importances_ssh1} and \ref{fourier_feature_importances_ssh2} show us that in both numerical experiments the zeroth component in wave vector space is the most important  for learning the topological phases, contributing about 49\% of reduction in information entropy in experiment 1 (section \ref{experiment_1}) and 95\% in experiment 2 (section \ref{experiment_2}). Let $x[m]$ be an eigenvector in real space. We can use equation \eqref{dft} to compute its wavevector space representation $k[n]$ and in particular its zeroth component,
\begin{equation}
\label{k_0} 
k[0] = \frac{1}{\sqrt{N}}\sum_{m=0}^{N-1}x[m].
\end{equation}
Equation \eqref{k_0} indicates that simply knowing the sum of components of the eigenvectors in real space may provide key information about the topological phases of SSH Hamiltonians. To visually examine this proposition, we can draw histograms  

The next most relevant components in wavevector space in both numerical experiments are the symmetric pair (2,98), which together reduce approximately 10\% of information entropy in experiment 1 (section \ref{experiment_1}) and 2.5\% in experiment 2 (section \ref{experiment_2}). The pair (2,98) in wavevector space leads us to another real space engineered feature,
\begin{equation}
\label{k_2_98} 
\frac{k[2]+k[98]}{\sqrt{2}} = \sqrt{\frac{2}{N}}\sum_{m=0}^{N-1}x[m]\cos(2m\omega_N).
\end{equation}
