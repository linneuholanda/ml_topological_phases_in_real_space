...
\section{Topological feature engineering and data compression}
...
\begin{equation}
\label{DCT}
\hat{x}_{c}[n] = x[0] + (-1)^n x[M-1] + \sum_{m=1}^{M-2} 2x[m]\cos\bigg(\frac{\pi}{M-1}nm\bigg), \quad n=0,\dots,M-1, 
\end{equation}
...
\begin{equation}
\label{DST}
\hat{x}_{s}[n+1] = \sum_{m=0}^{M-1} 2x[m+1]\sin\bigg(\frac{\pi}{M+1}(n+1)(m+1)\bigg), \quad n=0,\dots,M-1,
\end{equation} 
where $M = \frac{N}{2}-1$. 

We ran several numerical experiments to evaluate how the DCT and DST topological features defined in equations \eqref{DCT} and \eqref{DST} perform. The results obtained in each experiment are listed in table \ref{numerical_results}. These numerical experiments differed both on which topological features were used (whether DCT or DST topological features), as well as which features in real space were used to compute the engineered topological features. The results obtained in the numerical experiments of section \ref{numerical_experiments} are also listed in table \ref{numerical_results} for the sake of reference.  

\begin{table}
 \begin{tabular}{||c c c c c||} 
 \hline
 SSH system & Real space feat. & Wavevector space feat. & Val. Hamiltonian acc. & Test Hamiltonian acc. \\ % [0.5ex] 
 \hline\hline
 1 & All & None & 1 & 0.991 \\ 
 \hline
 1 & $\mathcal{S}_1$ & None & 0.985 & 0.985\\
 \hline
 1 & $\mathcal{S}'_1$ & None & 0.985 & 0.986\\
 \hline
 1 & DCT & DCT & 0.997 & 0.989 \\
 \hline
 1 & DST & DST & --- & --- \\
 \hline
 1 & $\mathcal{S}_1$ & $\mathcal{E}_1$ & --- & --- \\ 
 \hline
 1 & $\mathcal{S'}_1$ & $\mathcal{E}'_1$ & --- & --- \\ 
 \hline
 1 & $\mathcal{S}_1$ & $\mathcal{O}_1$ & --- & --- \\ 
 \hline
 1 & $\mathcal{S'}_1$ & $\mathcal{O}'_1$ & --- & --- \\ 
 \hline
 2 & All & None & --- & --- \\ 
 ...
 2 & DST & DST & --- & --- \\
 \hline
 2 & $\mathcal{S}_2$ & $\mathcal{E}_2$ & --- & --- \\ 
 \hline
 2 & $\mathcal{S'}_2$ & $\mathcal{E}'_2$ & --- & --- \\ 
 \hline
 2 & $\mathcal{S}_2$ & $\mathcal{O}_2$ & --- & --- \\ 
 \hline
 2 & $\mathcal{S'}_2$ & $\mathcal{O}'_2$ & --- & --- \\ 
 \hline
 
 \end{tabular}
\caption{\label{numerical_results}Your caption.}
\end{table}


Guaranteed to have Hermitian symmetry with respect to the lattice site at $\frac{N}{2}$. Furthermore, given that the lattices were closed with periodic boundary conditions, the   
%After computing the norm of each component of the resulting discrete information entropy wave function $\hat{\psi}_S[n]$, we recover the %information entropy signatures in wavevector space shown in figures \ref{fourier_feature_importances_ssh1} and %%\ref{fourier_feature_importances_ssh2}. 

Contrary to their real space counterparts, the wavevector space information entropy signatures are highly localized, which indicates that most of the topological information can be traced to a subset of wavevector modes. In fact, in figures \ref{fourier_feature_importances_ssh1} and \ref{fourier_feature_importances_ssh2} the reduction in information entropy is concentraded mostly on even wavevector modes. Furthermore, the wavevector space information entropy signatures are approximately even-symmetric with respect to the wavevector modes at 0 and $\frac{N}{2}$, a consequence of the fact that they are obtained as the DFT of a real signal with periodic boundary conditions. 

These (appproximate) symmetries of the topological signatures can guide us through engineering of new topological features from real space coordinates. Indeed, it is easy to verify that the DFT of a real signal $x[m]$ that is even-symmetric around 0 and $\frac{N}{2}$ is given by the Discrete Cosine Transform (DCT),
\begin{equation}
\label{DCT}
\hat{x}[n] = x[0] + (-1)^n x[M-1] + \sum_{m=1}^{M-2} 2x[m]\cos\Big(\frac{\pi}{M-1}nm\Big), \quad n=0,\dots,M-1, 
\end{equation}
where $M = \frac{N}{2}+1$. 

The DCT is often employed in several applications of Digital Signal Processing such as audio and image processing and, most importantly here, data compression. As equation \eqref{DCT} shows, the DCT profits from redundance of information in a real, even-symmetric signal, allowing us to write a signal of length $N$ in terms of $M = \frac{N}{2}+1$ features. 

We further reduce the number of engineered topological features by taking only even values of $n$ in the left-hand side of equation \eqref{DCT}. This is done in agreement with the observation that the information entropy signatures in figure \ref{fourier_feature_importances} are strongly localized on even wavevector modes. This leads us to our desired topological features,
\begin{equation}
\hat{x}[n] = x[0] + x[M-1] + \sum_{m=1}^{M-2} 2x[m]\cos\Big(\frac{2\pi}{M-1}nm\Big), \quad n=0,\dots,\frac{M-1}{2}.
\end{equation}  



 %the DFT of a real eigenvector $x[m]$\eqref{discrete_fourier_transform} o \eqref{in figures \ref{fourier_feature_importances_ssh1} and %\ref{fourier_feature_importances_ssh2}.  This gives us an opportunity to engineer new features in real space that carry more relevant %topological information than the simpler strategy of selecting the most important lattice sites in a real space information entropy %signature. We may write equation \eqref{discrete_fourier_transform} in matrix form as 
\begin{equation}
\label{dft}
\hat{\mathbf{\psi}}_S = \frac{1}{\sqrt{N}} \mathbf{\mathcal{F}}_{N}\mathbf{\psi}_S
\end{equation}
where we have used the Sylvester DFT matrix
\begin{equation}
\label{sylvester_dft}
\mathbf{\mathcal{F}}_{N} = \begin{bmatrix}
\omega_N^{0\cdot 0} & \omega_N^{0\cdot 1} & \omega_N^{0\cdot 2} &... & \omega_N^{0\cdot (N-1)}\\
\omega_N^{1\cdot 0} & \omega_N^{1\cdot 1} & \omega_N^{1\cdot 2} &... & \omega_N^{1\cdot (N-1)}\\
\omega_N^{2\cdot 0} & \omega_N^{2\cdot 1} & \omega_N^{2\cdot 2} &... & \omega_N^{2\cdot (N-1)}\\
\vdots & \vdots & \vdots & \cdots & \vdots\\ 
\omega_N^{(N-1)\cdot 0} & \omega_N^{(N-1)\cdot 1} & \omega_N^{(N-1)\cdot 2} &... & \omega_N^{(N-1)\cdot (N-1)}\\
\end{bmatrix}, \quad \text{with} \quad \omega_N = e^{-i\frac{2\pi}{N}}.
\end{equation}

When trying to engineer informative global topological features in real space, the information entropy signatures in wavevector space provide good hints of where to start.  Figures \ref{fourier_feature_importances_ssh1} and \ref{fourier_feature_importances_ssh2} show us that in both numerical experiments the zeroth component in wave vector space is the most important  for learning the topological phases, contributing about 49\% of reduction in information entropy in experiment 1 (section \ref{experiment_1}) and 95\% in experiment 2 (section \ref{experiment_2}). Let $x[m]$ be an eigenvector in real space. We can use equation \eqref{dft} to compute its wavevector space representation $\hat{k}[n]$ and in particular its zeroth component,
\begin{equation}
\label{x_hat_0} 
\hat{x}[0] = \hat{k}[0] = \frac{1}{\sqrt{N}}\sum_{m=0}^{N-1}x[m].
\end{equation}
Equation \eqref{x_hat_0} amounts to taking the sum of components of the eigenvectors in real space as a first engineered topological feature. 
%may provide key information about the topological phases of SSH Hamiltonians. To visually examine this proposition, we can draw histograms %-----  

Now let us consider the symmetric pairs $(m,N-m)$, with $m$ even. As figures \ref{fourier_feature_importances_ssh1} and \ref{fourier_feature_importances_ssh2} indicate, these pairs concentrate the remaining relevant topological information from the SSH systems investigated here. We can therefore engineer the topological features  

\begin{equation}
\label{x_hat_m} 
\hat{x}[n] = \frac{k[2n]+k[N-2n]}{2} = \sqrt{\frac{1}{N}}\sum_{m=1}^{\frac{N}{2}-1}x[2m]\cos(2mn\omega_N), \quad n = 1, 2, \cdots, \frac{N}{2}-1.
\end{equation}

Putting the engineered topological features from equations \eqref{x_hat_0} and \eqref{x_hat_m} together, we obtain a compressed version of the real space eigenvectors,





These  which together reduce approximately 10\% of information entropy in experiment 1 (section \ref{experiment_1}) and 2.5\% in experiment 2 (section \ref{experiment_2}). The pair (2,98) in wavevector space leads us to another real space engineered feature,
\begin{equation}
\label{k_m} 
\frac{k[m]+k[N-m]}{\sqrt{2}} = \sqrt{\frac{2}{N}}\sum_{m=0}^{N-1}x[m]\cos(2m\omega_N).
\end{equation}

The two engineered features, $k[0]$ and $\frac{k[2]+k[98]}{\sqrt{2}}$  
       
\begin{equation}
\label{matrix_discrete_fourier_transform}
\begin{bmatrix}
\hat{\psi}_S[0] \\
\hat{\psi}_S[1] \\
\vdots \\
...
Let us define a discrete wavevector space unit pulse as
 
\begin{equation}
\label{wavevector_unit_impulse}
\hat{\delta}^{(m)}_n = \hat{\delta}[n-m] =
\begin{cases}
    1, & \text{if } n=m\\
    0,              & \text{otherwise}.
\end{cases}
...
Then equation \eqref{discrete_fourier_transform} yields

\begin{equation}
\label{fourier_wavevector_unit_impulse}
\hat{\delta}^{(m)}_n = \hat{\delta}[n-m] =
\begin{cases}
 \frac{1}{\sqrt{N}}\sum_{l=0}^{N-1}x[l]e^{-\frac{2\pi i}{N} nl}, & \text{if } n=m\\
    0,              & \text{otherwise}
\end{cases}
\end{equation}
Let us consider the Inverse Discrete Fourier Transform (IDFT) formula, which allows us to invert equation



 \eqref{discrete_fourier_transform}

\begin{equation}
\label{inverse_discrete_fourier_transform}
\psi_S[n] = \frac{1}{\sqrt{N}}\sum_{m=0}^{N-1}\hat{\psi}_S[m]e^{\frac{2\pi i}{N} mn}.
\end{equation}

In both figures \ref{fourier_feature_importances_ssh1} and \ref{fourier_feature_importances_ssh2}, the main mode in wavevector space 




 main mode of the wavevector space information entropy signatures, which is  
